...
io.on('connection', socket => {
  ...
  socket.on('join', (user, callback) => {
    ...
    socket.emit('message:new', message('System', `Welcome, ${user.name}!`))
    socket.broadcast.to(user.room).emit('message:new', message('System', `${user.name} joined.`))
  })

  socket.on('message:create', (data, callback) => {
    if (!data.text || !data.name || !data.id) {
  ...
  socket.on('disconnect', () => {
    ...
    if (user) {
      io.to(user.room).emit('message:new', message('System', `${user.name}, left.`))
      io.to(user.room).emit('users:update', users.getByRoom(user.room))
    }
  })
})
